...
engine_SNC = create_engine('mssql+pyodbc://{server}:{port}/{db}?driver={dvr}&{auth}'
                          .format(server = 'sqlsrv-diari-ui-trv-ws.database.',
                                  port = '1433',
...
server = 'sqlsrv-diari-ui-trv-ws.database.windows.net'
port = '1433'
database = 'db-diari-ui-trv-ws-supernotariado'
...
# Conectarse a SQL Server usando pyodbc
try:
...
for f in files:
    ...
    if CATASTRO == True:
        ...
        df_Merge4.to_excel(os.path.join(path, str(start_id) + '_' + base_name, f'{base_name}_CATASTRO.xlsx') , index=False)
