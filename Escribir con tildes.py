# -*- coding: utf-8 -*-
"""
Created on Fri Dec 13 15:46:36 2024
...
engine_RUES = create_engine('mssql+pyodbc://{server}:{port}/{db}?driver={dvr}&{auth}'
                          .format(server = 'sqlsrv-diari-ui-trv-ws.database.windows.net',
                                  port = '1433',
...
for f in files:
    ...
    data = data[["IDENTIFICACIÓN"]]
    data["IDENTIFICACIÓN"] = data["IDENTIFICACIÓN"].astype(str)
    data["IDENTIFICACIÓN"] = data["IDENTIFICACIÓN"].str.replace('(\D+)',"")
    ...
    data.rename(columns = {'IDENTIFICACIÓN': 'IDENTIFICACIÓN'}, inplace = True)
    ...
    if SUPERNOT == True:
          ...
          for CC in data["IDENTIFICACIÓN"]:
              consulta_anotaciones = f'''select 
             db.[tipodocidentcons]
      		,db.[identifconsultada] AS IDENTIFICACIÓN
      		,db.[novedad]
      		,db.[idjurisdiccion]
      		,db.[jurisdiccion]
      		,db.[matricula]
              ...
              SELECT db.idLogConsulta, db.id, db.IdentifConsultada AS IDENTIFICACIÓN, db.IdJurisdiccion ,db.Matricula, db.novedad, db.Direccion, db.Chip, db.CedulaCatastral,
              ...
              appended_anotac['IDENTIFICACIÓN'] = appended_anotac['IDENTIFICACIÓN'].astype(str)
              propietarios = pd.read_sql(consulta_propietarios,engine_SNC)
              appended_propiet = appended_propiet.append(propietarios)
              appended_propiet['IDENTIFICACIÓN'] = appended_propiet['IDENTIFICACIÓN'].astype(str)
              print(f)
              print(CC)
              print(appended_anotac)
          
          df_Merge1 = pd.merge(data["IDENTIFICACIÓN"], appended_anotac, on = 'IDENTIFICACIÓN', how = 'left')
          df_Merge2 = pd.merge(data["IDENTIFICACIÓN"], appended_propiet, on = 'IDENTIFICACIÓN', how = 'left')
          
          df_Merge1.to_csv(os.path.join(path, str(start_id) + '_' + base_name, f'{base_name}_SUPERNOTARIADO_anotaciones.csv') , index=False)
          df_Merge2.to_csv(os.path.join(path, str(start_id) + '_' + base_name, f'{base_name}_SUPERNOTARIADO_propietarios_propiedades.csv') , index=False)
    
    if CATASTRO == True:
        
        for CC in data["IDENTIFICACIÓN"]:
            consulta_cat = f'''
            select pr.idLogConsulta, pr.identifConsultada AS IDENTIFICACIÓN, pr.matricula, pr.direccion_real,pr.chip,
        ...
        df_Merge4 = pd.merge(data["IDENTIFICACIÓN"], appended_cat, on = 'IDENTIFICACIÓN', how = 'left')
    ...
    if RUES == True:
        ...
        for CC in data["IDENTIFICACIÓN"]:
            ...
            query_rues = f'''SELECT
                [identConsultada] AS IDENTIFICACIÓN
            ...
            appended_rues['IDENTIFICACIÓN'] = appended_rues['IDENTIFICACIÓN'].astype(str)
            
        df_Merge4 = pd.merge(data["IDENTIFICACIÓN"], appended_rues, on = 'IDENTIFICACIÓN', how = 'left')
    ...
    if DAFP == True:
        ...
        for CC in data["IDENTIFICACIÓN"]:
            ...
            appended_dafp['IDENTIFICACIÓN'] = appended_dafp['IDENTIFICACIÓN'].astype(str)
            
        #df_ConsultaVinculaciones["IDENTIFICACION"] = df_ConsultaVinculaciones["IDENTIFICACION"].astype(int)
        df_Merge3 = pd.merge(data["IDENTIFICACIÓN"], appended_dafp, on = 'IDENTIFICACIÓN', how = 'left')
